...
# 1.5B fits comfortably in 8GB. The 7B needs ~15GB and will not fit.
MODEL = "deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B"
LAYER = 14            # this model has 28 layers; middle is a sensible default
...
# A hook grabs just the one layer we want. Asking for output_hidden_states
# would allocate all 28 layers for the whole sequence and blow up an 8GB card.
_captured = {}
...
def install_hook(model):
    ...
    # hidden_states[L] in HF is the output of decoder layer L-1.
...
def thinking_vector(tok, model, prompt):
    ...
    # Only re-run as far as we need: prompt plus the thinking tokens we keep.
...
def main():
    ...
    np.savez("results.npz", **vecs, pos_jailbreak=pos,
